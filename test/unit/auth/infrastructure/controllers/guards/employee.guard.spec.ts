import { EmployeeGuard } from '../../../../../../src/auth/infrastructure/controllers/guards/employee.guard';

describe('Employee Guard', () => {
    ...
    beforeEach(() => {
        ...
        guard = new EmployeeGuard(tokenService);
    });

    describe('Success', () => {
        it('should return true if the token is valid and contains the "employee" role', async () => {
            jest.spyOn(context, 'getRequest').mockReturnValue(request);
            jest.spyOn(tokenService, 'verifyToken').mockReturnValue({ role: 'employee' });
    ...
    describe('Failure', () => {
        ...
        it('should return false if token payload role is not employee', async () => {
            jest.spyOn(context, 'getRequest').mockReturnValue(request);
            jest.spyOn(tokenService, 'verifyToken').mockReturnValue({role: 'notEmployee'});
